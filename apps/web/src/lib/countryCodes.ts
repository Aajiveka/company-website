/**
 * Every ISO 3166-1 country and territory with an assigned E.164 calling code.
 *
 * `dial` includes the leading '+'. Several entries share one code on purpose — +1 covers the
 * whole North American Numbering Plan, +7 covers Russia and Kazakhstan, +44 covers the UK and
 * the Crown Dependencies — so `iso2` is the identity here, not `dial`.
 *
 * Uninhabited territories (Antarctic dependencies, Bouvet Island, the minor outlying islands)
 * are omitted: they have no subscriber numbers to enter.
 */
export interface Country {
  /** ISO 3166-1 alpha-2. The stable key — codes are not unique. */
  iso2: string;
  name: string;
  /** E.164 country calling code, with '+'. */
  dial: string;
}
...
export const COUNTRIES: Country[] = [
  ...
  { iso2: 'HK', name: 'Hong Kong SAR China', dial: '+852' },
  ...
  { iso2: 'MO', name: 'Macao SAR China', dial: '+853' },
  ...
  { iso2: 'MM', name: 'Myanmar (Burma)', dial: '+95' },
  ...
  { iso2: 'PS', name: 'Palestinian Territories', dial: '+970' },
...
/** The site's primary market, and what the API assumes when no code is sent. */
export const DEFAULT_COUNTRY_ISO2 = 'IN';
export const DEFAULT_DIAL_CODE = '+91';

/**
 * Flag as a pair of regional indicator symbols, derived from the ISO code rather than stored.
 * Platforms without flag glyphs (notably Windows) render the two letters instead, which is
...
/** First country carrying this dial code — used to show a flag for a stored '+91'-style value. */
