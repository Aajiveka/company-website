...
const MENUS: Record<RoleId, MenuItem[]> = {
  // Candidate sidebar. Entries whose screens are outside the "Aajiveka UI" Figma are
  // commented out to match their routes in router.tsx — a live link to a dead route would
  // land on the not-found page. Uncomment the route and the entry together to restore one.
  [Role.Subscriber]: [
