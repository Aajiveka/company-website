...
/** Fixed widths so opening accordion / scrollbar cannot shrink the rail. */
const EXPANDED = 'w-52 min-w-52 max-w-52';
const COLLAPSED = 'w-[4.5rem] min-w-[4.5rem] max-w-[4.5rem]';
...
export function EmployerSidebar({
  ...
  return (
    ...
    >
      <nav
        className={cn(
          'sidebar-scroll flex min-h-0 flex-1 flex-col gap-0.5 overflow-x-hidden overflow-y-auto [scrollbar-gutter:stable]',
          collapsed ? 'p-1.5' : 'p-2',
        )}
      >
          ...
          if (!hasChildren) {
            return (
              ...
              >
                ...
                {!!badge && (
                  <span className={cn('shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-semibold', brand.bgSoft, brand.text)}>
                    {badge}
                  </span>
                )}
              </NavLink>
          ...
          return (
            <div key={item.id} className={cn('rounded-lg', isOpen && 'bg-slate-50')}>
              <div className="flex items-center">
                ...
                <button
                  type="button"
                  className="mr-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-slate-400 hover:bg-slate-200/70 hover:text-slate-700"
              ...
              >
                <div className="overflow-hidden">
                  <div className="ml-3 space-y-0.5 border-l border-slate-200 py-1 pr-1.5 pb-1.5 pl-2.5">
      ...
      <button
        ...
        className={cn(
          'absolute top-[4.75rem] right-0 z-50 hidden h-7 w-7 translate-x-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm lg:flex',
          'hover:border-[#1A56DB]/40 hover:text-[#1A56DB]',
        )}
